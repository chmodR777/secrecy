...
public class BackUpBean extends BusinessLogic {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response, Parameters parameters)
        ...
        try {
            ...
            // 调用 mysql 的 cmd:
            Process child = rt
                    .exec(" mysqldump -u root --password=root his  --set-charset=utf8   ");// 设置导出编码为utf8。这里必须是utf8
            ...
            File file=new File("数据库备份"+fileName+".sql");
            if(!file.exists())
                file.createNewFile();
            ...
            String realName = "数据库备份"+fileName+".sql";
            File zipFile = new File("数据库备份"+fileName+".sql");
