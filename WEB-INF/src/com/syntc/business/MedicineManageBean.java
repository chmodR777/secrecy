...
public class MedicineManageBean extends BusinessLogic {
	/**
	 * 构造函数
	 */
	public MedicineManageBean() {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response, Parameters parameters) throws CommonException {
		...
		try {
			...
			sb_SQL.append("SELECT DISTINCT T_MEDICINE.C_MONTH ");
			sb_SQL.append("FROM T_MEDICINE ");
			sb_SQL.append("WHERE 1 = 1 ");
			if (p_Month.length() > 0) {
				sb_SQL.append("AND T_MEDICINE.C_MONTH like '%" + p_Month + "%' ");
			}
			...
			if (CommonConstants.CLDEF_DB_OK <= res.getStatus()) {
				...
				// 设置迁移首画面
				String str_URL = "/app/system/medicine_manage.jsp";
				// 画面迁移
			...
			System.out.println("MedicineManageBean:" + ex.toString());
			parameters.setParameters("results", "ForwardPage", CommonConstants.CLDEF_ERROR_PAGE);
		}
	}
